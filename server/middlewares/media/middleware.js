...
module.exports = function(CORE){
	...
	var _processValidFile = function(validation_result){
		return new Promise(function(resolve, reject){
			...
			var destination_folder = CORE.config.upload_folder_relative_to_root;
			var filename =  Date.now() + "_"+validation_result.file.originalname;

			var destination_URL = destination_folder + filename;
			...
			wr.on('finish', function(){
				...
				fs.unlink( validation_result.file.path, function(err){
					...
					resolve({
						...
						url: CORE.config.route_to_upload_folder + filename,
						alt: "",
